...
class SubClient(client.Client):
    ...
    def reize(path, size: int):
    	Image.open(path).resize((837, size)).save(path, format="png")
    	resize(path="image.png", size=736)
    	with open("image.png", "rb") as f:
    		full_embed(link, f, "message", "chatId")
    ...
    def send_active_obj(self, startTime: int = None, endTime: int = None, optInAdsFlags: int = 2147483647, tz: int = -timezone // 1000, timers: list = None, timestamp: int = int(timestamp() * 1000)):
        ...
        response = requests.post(f"{self.api}/x{self.comId}/s/community/stats/user-active-time", headers=headers.Headers(data=data, deviceId=self.device_id).headers, data=data, proxies=self.proxies, verify=self.certificatePath)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else: return response.status_code
    ...
    def invite_to_vc(self, chatId: str, userId: str):
        ...
        **Returns**
        ...
        response = requests.post(f"{self.api}/x{self.comId}/s/chat/thread/{chatId}/vvchat-presenter/invite/", headers=self.parse_headers(data=data).headers, data=data, proxies=self.proxies, verify=self.certificatePath)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
        else: return response.status_code
    ...
    def live_notify(self, chatId: str, userId: str):
        response = requests.delete(f"https://aminoapps.com/api-p/x{self.comId}/s/chat/thread/{chatId}/member/{userId}/invite-av-chat", headers=self.parse_headers(), proxies=self.proxies, verify=self.certificatePath)
